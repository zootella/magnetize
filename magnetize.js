...
function createDownloadLinks(urls) {
  ...
  for(var i = 0; i < types.length; ++i) {
    if(urlsOfType[types[i]].length > 0) {
      html += '<p style="margin: 0; padding-top: 2px; margin-left: 18px;font: 12px arial; font-weight: normal;"><a style="color: #2152a6;" href="' + generateMagnetUrl(urlsOfType[types[i]]) + '">Download ' + types[i] + ' Only (' + urlsOfType[types[i]].length + ')</a></p>';
    }
  }
  html += '</div>';
